...
const ingredients = db.prepare('SELECT * FROM recipe_ingredients WHERE recipe_id = ? ORDER BY sort_order').all(recipeId);

const pfSettings = db.prepare(
  'SELECT ps.* FROM preferment_settings ps JOIN recipe_ingredients ri ON ri.id = ps.ingredient_id WHERE ri.recipe_id = ?'
).all(recipeId);
const pfBps = db.prepare(
...
console.log('Built recipe: ' + recipe.name + ' (' + ingredients.length + ' ingredients)');
db.close();
