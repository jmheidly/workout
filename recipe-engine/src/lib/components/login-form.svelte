  ...
  async function signInWithPasskey() {
    ...
    try {
      ...
      if (!optionsRes.ok) {
        ...
        passkeyError = err.error || 'Failed to start passkey login'
        return
      }
      ...
      if (!verifyRes.ok) {
        ...
        passkeyError = err.error || 'Passkey login failed'
        return
      }
...
<div class="flex flex-col gap-6">
  <Card.Card>
    <Card.CardHeader class="text-center">
      ...
      <Card.CardDescription>Login with your email, Google, or passkey</Card.CardDescription>
    </Card.CardHeader>
    <Card.CardContent>
      <div class="grid gap-6">
        ...
        <div class="relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t after:border-border">
          <span class="relative z-10 bg-card px-2 text-muted-foreground">OR</span>
        </div>
        ...
        {#if passkeySupported}
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4"><path d="M7 11V7a5 5 0 0 1 9.9-1"/><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><circle cx="12" cy="16" r="1"/></svg>
